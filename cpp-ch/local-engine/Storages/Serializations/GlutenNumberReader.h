...
inline bool checkComma(DB::ReadBuffer & buf)
{
...
static inline bool readUIntTextUpToNSignificantDigits(T & x, DB::ReadBuffer & buf)
{
    ...
    if (buf.position() + N <= buf.buffer().end())
    {
        for (size_t i = 0; i < N; ++i)
        {
            ...
            {
                if (*buf.position() == ',' && !checkComma(buf))
                    return true;
            }
            else
                return true;
...
template <typename T>
inline bool readGlutenFloatTextFastImpl(T & x, DB::ReadBuffer & in)
{
    ...
    if (!local_engine::readUIntTextUpToNSignificantDigits<significant_digits, true>(before_point, in))
        return false;
    ...
    if (checkChar('.', in))
    {
        ...
        local_engine::readUIntTextUpToNSignificantDigits<significant_digits>(after_point, in);
    ...
    if (checkChar('e', in) || checkChar('E', in))
    {
        ...
        local_engine::readUIntTextUpToNSignificantDigits<4>(exponent, in);
...
template <typename T>
bool readGlutenIntTextImpl(T & x, DB::ReadBuffer & buf)
{
    ...
    while (!buf.eof())
    {
        ...
        else if (*buf.position() == ',' && !checkComma(buf))
        {
