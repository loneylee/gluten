...
template <typename T>
inline void readGlutenFloatText(T & x, DB::ReadBuffer & buf)
{
    {
        char * pos = buf.position();
        if (local_engine::readGlutenFloatTextFastImpl(x, buf))
            return;
        else
            buf.position() = pos;
